...
export function SectionModal({ 
  ...
  const t = useTranslations('components.createSection');
  ...
  const createSectionMutation = trpc.section.create.useMutation({
    ...
    onError: (error) => {
      toast.error(error.message || t('toasts.errorCreate'));
    }
  });

  const updateSectionMutation = trpc.section.update.useMutation({
    onSuccess: (data) => {
      toast.success(t('toasts.updated', { name: data.name }));
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="sectionName">{t('fields.name')}</Label>
            <Input
              id="sectionName"
              value={sectionName}
